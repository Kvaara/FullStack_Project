...
const courseFound = (course, student) => {
  ...
  // TODO student.courses.push(course);
  student.courses += `${course.course_name} | `;
  ...
  fetch(`/register/${student.student_number}`, {
    ...
    .then(() => {
      alert(
        `${student.first_name} ${student.last_name} has been registered to '${course.course_name}' course.`
      );
    })
    .catch((error) => {
      throw error;
...
const courseNotFound = (course) => {
  alert(
    `No courses named '${course}' found. Make sure that you have typed in the course name correctly.`
  );
};

const registering = (student, index) => {
